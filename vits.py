...
from lib.misc import get_2d_sincos_pos_embed
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None, init_values=None,
                 act_layer=None, weight_init='', class_attention_layers=2):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], init_values=None, norm_layer=norm_layer, act_layer=act_layer)
        ...
        self.blocks_token = nn.ModuleList([
            ...
            for i in range(class_attention_layers)])
    ...
    def forward_features(self, x, masks=None, mask_ratio=0.):
        ...
        # ExtreMA follows the CaiT-style architecture
        #x = torch.cat((cls_token, x), dim=1) 
        x = self.pos_drop(x + self.pos_embed)
        # for student
        if masks is not None and self.student:
            cls_token = self.cls_token.expand(x.shape[0] * len(masks), -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
            x_list = []
            for mask in masks:
